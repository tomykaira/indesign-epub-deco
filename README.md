...
EPUB の本文を少しだけ加工し、前後に画像ページを挿入して別ファイルとして書き出すツールです。

主にできること:

- 本文末尾/先頭に画像ページ（1画像 = 1ページ）を挿入
- 画像を指定の総ピクセル数に揃えて再圧縮
- 指定の色指定を含む CSS 宣言を削除（`border-color` など複合プロパティは破壊しません）
- 特定のクラスを持つ段落の自動字下げを先頭文字に応じて無効化
- InDesign 出力で本文末尾にまとめて出力される「全画面挿絵」を、ページ番号の欠落位置へ**自動再配置**
- EPUB3 必須の更新日時 (`dcterms:modified`) を現在時刻に更新

出力ファイル名は `<元のepubのファイル名>-concat.epub` となり、設定ファイル (JSON) と同じフォルダに置かれます。同名のファイルがあれば上書きします。
...
- 既に「Microsoft Store 版」の ImageMagick（`%LOCALAPPDATA%\Microsoft\WindowsApps\magick.exe`）が入っている場合、このツールからは起動できません。後述の **magick が起動できないとき** を参照してください。
...
| `resize` | 必須 | 画像の目標**総ピクセル数**（幅×高さ）。例 `1920000` は約 1600×1200 相当。縦横比は保たれます。 |
| `quality` | 必須 | JPEG 等の画質（1〜100、目安 80〜90）。 |
| `indentSkipChars` | 任意 | 字下げを無効化する対象の先頭文字を上書き。例: `"「『？！"`。既定値は `「『〈《【〔（(？！・`。 |
| `indentClassName` | 任意 | 字下げ無効化の対象とする `<p>` のクラス名。既定値は `本文-自動字下げ`。一般的な EPUB では変更不要です。 |
| `imageClassName` | 任意 | 画像自動再配置の対象とする `<div>` のクラス名。既定値は `全画面挿絵`。InDesign 出力の標準的な EPUB では変更不要です。 |
| `magick` | 任意 | ImageMagick の `magick.exe` の**絶対パス**。通常は不要（PATH から自動発見）。後述のトラブル対処で使います。 |
...
本ツールは SVG で画像をアスペクト比を保ったまま表示するため、歪みは発生しません。もし縦横比がおかしく見える場合は、元画像自体の縦横比を確認してください。`resize` は総ピクセル数なので縦横比には影響しません。

### 文字が細く表示される／色が気になる

`color: #231815`（暗めのグレー）の CSS 宣言を削除することで、端末のダークモード等で読みやすくなる想定です。この挙動は現状固定です。該当 CSS ファイルが無い EPUB では警告のみ出して続行します。`border-color` など `-color` で終わる複合プロパティは対象外なので、枠線の色などは維持されます。
...
- 出力は元の EPUB とは別ファイルとして書き出されるので、元 EPUB は変更されません。
- 画像は一時フォルダ（OS の tmp 配下）にコピーしてからリサイズするため、元画像も変更されません。
- 処理が途中で失敗した場合、出力ファイルは生成されません（不完全なファイルが残らないよう、書き込み完了後にアトミックリネームしています）。

## ライセンス等

本ツール自体は私的利用を前提とした小さなスクリプト群です。加工に使う EPUB のライセンス・再配布の可否は各自の責任で確認してください。
